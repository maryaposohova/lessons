...
ход: 4
   ...
   Закл. частью у нас является проверка , кто же победил? Хорошо, у нас  отрисовываются, ставить мы можем 
только на свододные ячейки, если ячейка занята, соответственно. ставить мы не можем. 
   Но...даже если мы заполним все поле крестиками. либо ноликами, нам тяжело будет определить победителя.
Естественно, чтобы с этим справиться, нам нужно выполнить проверку, проверку какого рода?
т.е нам нкжно взять наше поле area[row][column] , где будет номер area[0][0] == 'х', проверить, что там будет крестик, 
и в area[0][0] == 'х' and area[0][1] == 'х' and area[0][2] == 'х', т.е. выполнить проверку такого рода, что в первой 
строчке во всех столбцах будет находиться 'х', тогда это победа крестиков. Но нам нужно учесть все варианты. Нам нужна 
точно такая же запись с ноликами. И не только в строчку, но еще и по диагонали и по вертикали. По горизонтали также, 
но уже на других строчках.
Здесь нам нужно выполнить такую простую проверку, но много раз. Поэтому (он удалил строчку написанную для проверки 
первой строки) и заранее приготовил функцию.
...
...
    ...
    draw_area()
    area[0][0] == 'х' and area[0][1] == 'х' and area[0][2] == 'х'  # удалил строчку, и приготовил функцию check_winner
    
def check_winner():
    if area[0][0] == 'x' and area[0][1] == 'x' and area[0][2] == 'x':         # строки x
        return 'x'
    if area[1][0] == 'x' and area[1][1] == 'x' and area[1][2] == 'x':
        return 'x'
    if area[2][0] == 'x' and area[2][1] == 'x' and area[2][2] == 'x':
        return 'x'
    if area[0][0] == 'x' and area[1][0] == 'x' and area[2][0] == 'x':         #  столбцы x
        return 'x'
    if area[0][1] == 'x' and area[1][1] == 'x' and area[2][1] == 'x':
        return 'x'
    if area[0][2] == 'x' and area[1][2] == 'x' and area[2][2] == 'x':
        return 'x'
    if area[0][0] == 'x' and area[1][1] == 'x' and area[2][2] == 'x':         #  диагональ x
        return 'x'
    if area[0][2] == 'x' and area[1][1] == 'x' and area[2][0] == 'x':
        return 'x'
    if area[0][0] == '0' and area[0][1] == '0' and area[0][2] == '0':         # строки 0
        return '0'
    if area[1][0] == '0' and area[1][1] == '0' and area[1][2] == '0':
        return '0'
    if area[2][0] == '0' and area[2][1] == '0' and area[2][2] == '0':
        return '0'
    if area[0][0] == '0' and area[1][0] == '0' and area[2][0] == '0':         # столбцы 0
        return '0'
    if area[0][1] == '0' and area[1][1] == '0' and area[2][1] == '0':
        return '0'
    if area[0][2] == '0' and area[1][2] == '0' and area[2][2] == '0':
        return '0'
    if area[0][0] == '0' and area[1][1] == '0' and area[2][2] == '0':          # диагональ 0
        return '0'
    if area[0][2] == '0' and area[1][1] == '0' and area[2][0] == '0':
        return '0'
    return '*'

Здесь у меня возвращающая функция. 
Если вспомить, у нас функция оставляет на месте своего вызова оставляет какое-то значение. И когда я буду вызывать 
эту функцию, она мне будет оставлять либо крестик, либо нолик. Если никто не победил, она будет оставлять звездочку 
(для этого в конце функции return '*', возврат *).

А у нас функция является объектом, тем более возвращающая функция оставляет нам какое-то значение. Это значение я 
могу либо сохранять в переменную, либо использовать при вычислении и в моем случае, 
я ее хочу использовать для сравнения.
 Если моя функция чек_виннер  оставит крестик, то я напишу, что это победа крестика и дальше вспоминаем, 
 что нам нет смысла дальше продолжать игру, и нам нужно выйти из цикла, используем команду брейк
    if check_winner() == 'x':
    print('Победа крестиков')
    break
Победа крестиков готова. Это дело можно продублировать, поменнять для ноликов
    if check_winner() == '0':
    print('Победа ноликов')
    break
Ну а если чек_виннер оставит нам звездочкн и у нас тере == 9, то мы напишем, что это ничья.
    if check_winner() == '*' and turn == 9:
    print('Ничья')
    
    
    Сейчас на этом занятии мы разобрали один из возможных способов реализации игры крестики-нолики.
    На самом деле их существует гораздо больше. И сущесвтвуют более гибкие, более компактные варианты реализации 
    данной игры, но я постарался выбрать максимально простой вариант для реализации и наибольшим количеством 
    повторяющихся элементов
    Используем фор, иф, элс, брейк, континью и даже функции, о которых мы немного, но успели поговорить также 
    в этом модуле. 
    Но здесь открыты возможности для расширения, поэтому настаиваю попробовать реализовать крести-нолики по-своему.
"""


# Возвращающая функция для проверки
def check_winner():
    if area[0][0] == 'x' and area[0][1] == 'x' and area[0][2] == 'x':  # строки x
        return 'x'
    if area[1][0] == 'x' and area[1][1] == 'x' and area[1][2] == 'x':
